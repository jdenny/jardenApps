...
/*
DivideCell=========
Proposal: aminoAcid, when first created, belongs to cell; when added
to protein, it then belongs to that protein; AminoAcid.getCell() now
gets it from the protein.
Protein manages its own thread; don't start DivideCell if one
already running; Protein has methods start() & stop()

issues: cells moved to new cell need to be stopped
(note that adding them to the new cell restarts them)
Note that DivideCell could itself be one of the proteins to be moved!

Options:
1. each protein manages its own thread; this gives DivideCell
the flexibility to tell the required proteins to stop; add methods
to Protein: stop() & start(); sequence is now:
determine which proteins to move
1a. don't start DivideCell if one already running; this is preferable,
    as otherwise both proteins would start to divide the cell!
1b. move the DivideCell that is not the current one

2. The cell itself determines that it is ready to divide, and then
   calls DivideCell

3. Ditch the whole idea of threads and simply use callbacks;
   wowabaweeba!
   protein needs Adenine
   if one available: use it
   otherwise: wait for callback

for each:
    stop()
    setCell(daughterCell)
    start()
